...
parser.add_argument('--check_blender', type=boolean_string, default=False)
parser.add_argument("--blender_ckpt", type=str, default=None)
...
if str(config['model_name']) in ['gate_blenderbot_small', 'gate_blenderbot_small_gate_loss','gate_blenderbot_small_gate_loss_freeze']:
    print("--------------------------------------------------------------")
...
try:
    print(config['st_config'])
except:
    config['st_config'] = None
...
print("Model Loaded from ckpt: ", args.load_checkpoint)
print("---------------------------------------------------------------------------------")
...
for infer_idx, infer_input_file in enumerate(args.infer_input_file):
    ...
    for batch, posts, references, sample_ids in infer_dataloader:
        ...
        for idx in range(len(sample_ids)):
            ...
            other_res_to_append = {}
            other_res_to_append['gate_values'] = []
            ...
            # other_res_to_append['before_ffn'] = toker.decode(encoded_info['before_ffn'][idx]+ len(toker) - 8).replace(' ', '')
            # other_res_to_append['after_ffn'] = toker.decode(encoded_info['after_ffn'][idx]+ len(toker) - 8).replace(' ', '')            
            # other_res_to_append['gate_values'].append(str(np.array(encoded_info['gate_values']).tolist()))
    ...
    if not os.path.exists(save_dir):
        print(f"Attempting to create directory for path: {os.path.dirname(save_dir)}")
        os.mkdir(save_dir)
