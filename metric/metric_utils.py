...
class BERT_(nn.Module):
    def __init__(self, model_name_or_path, cache_dir, config):
        super().__init__()
        self.encoder = RobertaModel.from_pretrained(model_name_or_path, from_tf=bool('.ckpt' in model_name_or_path), config=config, cache_dir=cache_dir)
...
class InitModel(object):
    def __init__(self):
        ...
        self.tokenizer.add_special_tokens({'additional_special_tokens':['[goal]','[user]','[system]','[table]','[paragraph]','[scale]']})
...
def mi_metric(data_file, output_file, res_file, use_ucm=False, use_ref=False):
            ...
            for i in range(len(dial)):
                ...
                if i > 0:
                    ...
                    if use_ucm:
                        ...
                        if use_ref:
                            if turn['strategy'] in ['Question','Restatement or Paraphrasing','Providing Suggestions','Information']:
                                init = 'I'
                                metrics['pro'].append(1)
                            else:
                                init = 'N'
    ...
    mi_res['inf_all'] = float(sum(metrics['inf']['I'])+sum(metrics['inf']['N']))/len(metrics['inf']['I']+metrics['inf']['N'])
    mi_res['rep_i'] = float(sum(metrics['rep']['I']))/len(metrics['rep']['I'])
    mi_res['rep_n'] = float(sum(metrics['rep']['N']))/len(metrics['rep']['N'])
    mi_res['rep_all'] = float(sum(metrics['rep']['I'])+sum(metrics['rep']['N']))/len(metrics['rep']['I']+metrics['rep']['N'])
    mi_res['rel_i'] = float(sum(metrics['rel']['I']))/len(metrics['rel']['I'])
    mi_res['rel_n'] = float(sum(metrics['rel']['N']))/len(metrics['rel']['N'])
    mi_res['rel_all'] = float(sum(metrics['rel']['I'])+sum(metrics['rel']['N']))/len(metrics['rel']['I']+metrics['rel']['N'])
    print(mi_res)
    return mi_res
...
if __name__=='__main__':
    ...
    USi_model = USi()
    emo_model = EmoModel()
    context = {"situation": "I am always depressed and the upcoming holidays are making it a lot worse.", "dialog": [{"text": "Hello. How are you today?", "speaker": "sys", "strategy": "Others"}, {"text": "hi i am okay, a little bit sad though", "speaker": "usr"}, {"text": "Okay. I am very sorry to hear that! Do you want to tell me more about that?", "speaker": "sys", "strategy": "Question"}, {"text": "Well with the holidays coming up i have been very stressed and nervous about what i am going to do", "speaker": "usr"}, {"text": "i am very sorry to hear that. have you been able to go out for a walk?", "speaker": "sys", "strategy": "Affirmation and Reassurance"}]}
    feedback = USi_model.predict(context)
    print(feedback)
    print("Yes. I am a college student and I live on campus. The day before I moved in my dad said some unforgivable things to me and we haven't spoke since")
    print(emo_model.predict("Well with the holidays coming up i have been very stressed and nervous about what i am going to do"))
    print(emo_model.predict(feedback))
    print(emo_model.predict("Yes. I am a college student and I live on campus. The day before I moved in my dad said some unforgivable things to me and we haven't spoke since"))
